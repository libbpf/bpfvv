...
const createApp = (url: string) => {
    ...
    const gotoLineInput = document.getElementById(
        "goto-line",
    ) as HTMLInputElement;
    const gotoLineButton = document.getElementById(
        "goto-line-btn",
    ) as HTMLButtonElement;
    const gotoEndButton = document.getElementById(
        "goto-end",
    ) as HTMLButtonElement;
    ...
    const lineNumbersPc = document.getElementById(
        "line-numbers-pc",
    ) as HTMLElement;
    const lineNumbersIdx = document.getElementById(
        "line-numbers-idx",
    ) as HTMLElement;
    const dependencyArrows = document.getElementById(
        "dependency-arrows",
    ) as HTMLElement;
    ...
    const logLines = document.getElementById(
        "formatted-log-lines",
    ) as HTMLElement;

    const exampleLink = document.getElementById(
        "example-link",
    ...
    const mostRecentBpfState = (
        ...
        for (let i = idx; i >= 0; i--) {
            ...
            if (bpfState) return { state: bpfState, idx: i };
        }
        return { state: initialBpfState(), idx: 0 };
    };

    const memSlotDependencies = (
        state: AppState,
        idx: number,
        memSlotId: string,
    ): Set<number> => {
    ...
    const collectMemSlotDependencies = (
        state: AppState,
        memSlotId: string,
    ): number[] => {
        ...
        const deps = memSlotDependencies(
            state,
            state.selectedLineIdx,
            memSlotId,
        );
    ...
    const updateSelectedLineHint = (idx: number): void => {
        const hintLine = document.getElementById(
            "hint-selected-line",
        ) as HTMLElement;
    ...
    const setSelectedLine = (idx: number, memSlotId: string = ""): void => {
        ...
        if (memSlotId && memSlotId !== "MEM") {
            ...
            state.memSlotDependencies = collectMemSlotDependencies(
                state,
                memSlotId,
            );
        } else {
    ...
    const logLineMouseOver = (lineElement: HTMLElement): void => {
        ...
        if (lineElement) {
            const idx = parseInt(
                lineElement.getAttribute("line-index") || "0",
                10,
            );
            ...
            hintLine.innerHTML =
                '<span style="color: transparent;">hint text</span>';
        }
    };
    ...
    const depArrowMouseOver = (e: MouseEvent): void => {
        ...
        if (isVisible(prev)) {
            ...
            if (idx < mid) setTargetToPrev();
            else setTargetToNext();
        }
    };
    ...
    const logLineMouseOut = (logLine: HTMLElement): void => {
        if (logLine) logLine.classList.remove("hovered-line");
    };
    ...
    const setLineHighlightClass = (
        ...
        if (highlightClass) div.classList.add(highlightClass);
    };
    ...
    const bpf2bpfCallInstructionHtml = (
        line: ParsedLine,
        frame: number,
    ): string => {
    ...
    const indent = (s: string, depth: number): string => {
        for (let i = 0; i < depth; i++) s = "    " + s; // 4 spaces
        return s;
    };
    ...
    const logLineDiv = (line: ParsedLine, bpfState: BpfState): HTMLElement => {
        ...
        else setLineHighlightClass(div, "normal-line");
    ...
    const loadComplete = async (state: AppState): Promise<void> => {
        await Promise.all([updateView(state), updateLoadStatus(100, 100)]);
        gotoEnd();
    };
    ...
    const textDiv = (text: string, styleClass: string = ""): HTMLElement => {
        const div = document.createElement("div");
        div.innerHTML = text;
        if (styleClass) div.classList.add(styleClass);
        return div;
    };
    ...
    const loadRawLine = (state: AppState, rawLine: string) => {
        ...
        const bpfState = nextBpfState(
            mostRecentBpfState(state, idx).state,
            parsedLine,
        );
        const pcText =
            typeof parsedLine.bpfIns?.pc === "number"
                ? `${parsedLine.bpfIns.pc}:`
                : "\n";
    ...
    const resetSingleLineFormat = async (
        state: AppState,
        div: HTMLElement,
    ): Promise<void> => {
        ...
        else setLineHighlightClass(div, "ignorable-line");
    };

    const updateSingleLineFormat = async (
        state: AppState,
        div: HTMLElement,
    ): Promise<void> => {
        resetSingleLineFormat(state, div);
        const selectedLine = state.lines[state.selectedLineIdx];
        const isSelectedLineParsed =
            selectedLine?.bpfIns || selectedLine?.bpfStateExprs;
        const idx = contentLineIdx(div);
        ...
        if (
            idx === state.selectedLineIdx ||
            state.memSlotDependencies.includes(idx)
        ) {
            const memSlotSpan = div.querySelector(
                `#${state.selectedMemSlotId}`,
            ) as HTMLElement;
    ...
    const getVisibleIdxRange = (): [number, number] => {
        ...
        const relativeStart =
            (containerRect.top - linesRect.top) / linesRect.height;
        const relativeEnd =
            relativeStart + containerRect.height / linesRect.height;
    ...
    const memSlotDisplayValue = (
        state: AppState,
        memSlotId: string,
        idx: number,
    ): string => {
        const { state: bpfState, idx: bpfStateIdx } = mostRecentBpfState(
            state,
            idx,
        );
        ...
        switch (value?.effect) {
            ...
            case Effect.UPDATE:
                ...
                else if (newVal) content = `${oldVal} ${RIGHT_ARROW} ${newVal}`;
                else
    ...
    const updateStatePanel = async (state: AppState): Promise<void> => {
        const { state: bpfState, idx } = mostRecentBpfState(
            state,
            state.selectedLineIdx,
        );
    ...
    const scrollToLine = async (idx: number): Promise<void> => {
        ...
        const relativePosition =
            normalIdx(idx - page * 0.618) / state.lines.length;
    ...
    const handleKeyDown = async (e: KeyboardEvent) => {
        ...
        if (
            state.selectedLineIdx < minIdx + 8 ||
            state.selectedLineIdx > maxIdx - 8
        ) {
